...
% Semester Genap 2025/2026 | Kelompok 08
%
% Kompilasi:
%   xelatex laporan-akhir-kel08.tex
%   biber   laporan-akhir-kel08
%   xelatex laporan-akhir-kel08.tex
%   xelatex laporan-akhir-kel08.tex
%
...
\hypersetup{
  ...
  urlcolor=blue,
  citecolor=black,
  pdftitle={Laporan Akhir - Klasifikasi DDoS LSTM/GRU/RF - Kelompok 08},
  pdfauthor={Kelompok 08}
}
...
\textbf{Sistem Klasifikasi DDoS (End-to-End)} \\[0.8em]
\texttt{CIC-DDoS2019 Parquet} $\to$ Label Normalization (binary) \\[0.3em]
$\downarrow$ \\[0.3em]
Feature Selection (16 fitur CICFlowMeter) $\to$ Drop NaN/Inf \\[0.3em]
$\downarrow$ \\[0.3em]
Stratified Split 70/15/15 (seed=42) \\[0.3em]
$\downarrow$ \\[0.3em]
StandardScaler (fit train, transform val/test) \\[0.3em]
$\downarrow$ \\[0.3em]
\textbf{LSTM/GRU:} Sliding Window (seq\_len=10) $\to$ (n, 10, 16) \\
\textbf{RF:} Flat vector $\to$ (n, 16) \\[0.3em]
$\downarrow$ \\[0.3em]
Training $\to$ CrossEntropyLoss / RF fit $\to$ Checkpoint \\[0.3em]
$\downarrow$ \\[0.3em]
Evaluasi: Accuracy, F1, FPR, AUC-ROC
}}
...
\begin{figure}[h!]
\centering
\fbox{\parbox[c][7cm][c]{0.75\textwidth}{\centering
\textbf{Flowchart Training LSTM/GRU} \\[0.5em]
MULAI $\to$ Set seed (42) $\to$ Load dataset \\[0.3em]
$\downarrow$ \\[0.3em]
Preprocessing \& Sliding Window \\[0.3em]
$\downarrow$ \\[0.3em]
Inisialisasi model, optimizer, scheduler \\[0.3em]
$\downarrow$ \\[0.3em]
\textbf{Loop Epoch:} \\
train\_one\_epoch() $\to$ evaluate\_epoch() \\
Jika val\_F1 $>$ best\_val\_F1: save checkpoint \\
scheduler.step(val\_F1) \\[0.3em]
$\downarrow$ \\[0.3em]
Load best checkpoint $\to$ Evaluasi test set \\[0.3em]
$\downarrow$ \\[0.3em]
Simpan metrik ke CSV $\to$ SELESAI
}}
...
Klaim penguasaan konsep per anggota dideklarasikan dalam berkas \texttt{metadata-laporan-akhir-kel08.json} yang merupakan bagian tidak terpisahkan dari laporan ini.
...
\textit{[Diisi oleh Syahmi Hamdani]}
...
\textit{[Diisi oleh Christover Angelo]}
...
\textit{[Diisi oleh Matthew Immanuel]}
...
Berikut adalah isi berkas \texttt{metadata-laporan-akhir-kel08.json} yang dikumpulkan bersama laporan ini.
...
\begin{lstlisting}[language=json]
{
  ...
  "klaim_bonus": ["B1", "B3"],
  "tautan": {
    "github": "https://github.com/calvinkatoroy/tugas-akhir-ai-kel-08",
...
\begin{description}[leftmargin=5cm,labelwidth=4.5cm]
    \item[GitHub repo:]   \url{https://github.com/calvinkatoroy/tugas-akhir-ai-kel-08}
...
\textbf{Status akses:} repositori bersifat publik; Colab dapat diakses oleh siapapun yang memiliki tautan. Sudah diverifikasi terbuka per 25 Mei 2026.
...
Email            & : calvin.katoroy05@gmail.com \\
\end{tabular}
...
\begin{itemize}[leftmargin=2em, label={$\square$}, itemsep=0.8em]
    ...
    \item \textbf{Option B --- Limited Participation.} I consent to the use of my report, metadata, and audio recording, \textbf{but I do not consent to the video recording} being used for research.
    \item \textbf{Option C --- Decline Participation.} I decline to participate in the research.
\end{itemize}
...
\begin{itemize}[leftmargin=2em, label={$\square$}, itemsep=0.8em]
    \item \textbf{Option A --- Full Participation.}
    \item \textbf{Option B --- Limited Participation.}
    \item \textbf{Option C --- Decline Participation.}
\end{itemize}
...
Email            & : \textit{[diisi oleh Christover]} \\
\end{tabular}
...
Email            & : \textit{[diisi oleh Matthew]} \\
\end{tabular}
