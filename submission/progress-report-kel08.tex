...
Repository: \url{https://github.com/calvinkatoroy/tugas-akhir-ai-kel-08}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Tingkat ketergantungan:} rendah (estimasi $\sim$10\% output berasal dari AI)
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Kode/modul:} \texttt{src/preprocessing.py}, \texttt{notebooks/01\_eda.ipynb}, \texttt{notebooks/02\_preprocessing.ipynb}
    \item \textbf{Bagian arsitektur:} Pipeline preprocessing \textit{end-to-end} (label normalization, feature selection, StandardScaler, stratified split, sliding window)
    \item \textbf{Bagian analisis:} EDA distribusi kelas, korelasi antar fitur, justifikasi pemilihan 16 fitur CICFlowMeter, verifikasi tidak ada Inf/NaN setelah Kaggle cleaning
    \item \textbf{Bagian laporan:} Bab III.1 (dataset), Bab III.2 (pra-pemrosesan)
\end{itemize}

\subsection*{IV.2.2 Pernyataan Penggunaan AI \textit{Assistant}}
\begin{itemize}[leftmargin=*]
    \item \textbf{Tool yang dipakai:} Claude Sonnet 4.6 (Claude Code CLI, Anthropic)
    \item \textbf{Untuk bagian apa:} Debugging sesekali pada operasi Pandas dan visualisasi EDA
    \item \textbf{Tingkat ketergantungan:} rendah (estimasi $\sim$10\% output berasal dari AI)
    \item \textbf{Pernyataan:} Saya memahami dan dapat mempertanggungjawabkan setiap baris kode dan teks yang saya klaim sebagai bagian saya.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Kode/modul:} \texttt{src/models/rf\_model.py}, \texttt{src/evaluate.py}, \texttt{notebooks/05\_rf\_training.ipynb}, \texttt{notebooks/08\_evaluation.ipynb}
    \item \textbf{Bagian arsitektur:} Implementasi Random Forest baseline, pipeline evaluasi (classification\_report, confusion matrix, ROC/AUC untuk semua model)
    \item \textbf{Bagian analisis:} Analisis performa per-kelas, perbandingan metrik antar model, visualisasi confusion matrix
    \item \textbf{Bagian laporan:} Bab III.4 (implementasi model), Bab III.5 (hasil eksperimen)
\end{itemize}

\subsection*{IV.3.2 Pernyataan Penggunaan AI \textit{Assistant}}
\begin{itemize}[leftmargin=*]
    \item \textbf{Tool yang dipakai:} Claude Sonnet 4.6 (Claude Code CLI, Anthropic)
    \item \textbf{Untuk bagian apa:} Debugging sesekali pada plotting confusion matrix dan ROC curve
    \item \textbf{Tingkat ketergantungan:} rendah (estimasi $\sim$10\% output berasal dari AI)
    \item \textbf{Pernyataan:} Saya memahami dan dapat mempertanggungjawabkan setiap baris kode dan teks yang saya klaim sebagai bagian saya.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Kode/modul:} \texttt{src/utils.py}, bagian visualisasi di \texttt{notebooks/08\_evaluation.ipynb}
    \item \textbf{Bagian arsitektur:} Utility functions (seed setting, tqdm logging helpers, comparative plot functions), experiment logging ke \texttt{results/metrics\_summary.csv}
    \item \textbf{Bagian analisis:} Overlay ROC curves, loss curves training LSTM/GRU, tabel hyperparameter experiment
    \item \textbf{Bagian laporan:} Bab II.1 (studi literatur), Ringkasan Eksekutif, analisis etika \& bias (untuk laporan akhir)
\end{itemize}

\subsection*{IV.4.2 Pernyataan Penggunaan AI \textit{Assistant}}
\begin{itemize}[leftmargin=*]
    \item \textbf{Tool yang dipakai:} Claude Sonnet 4.6 (Claude Code CLI, Anthropic)
    \item \textbf{Untuk bagian apa:} Pengecekan tata bahasa sesekali pada penulisan laporan
    \item \textbf{Tingkat ketergantungan:} rendah (estimasi $\sim$10\% output berasal dari AI)
    \item \textbf{Pernyataan:} Saya memahami dan dapat mempertanggungjawabkan setiap baris kode dan teks yang saya klaim sebagai bagian saya.
\end{itemize}
...
Berikut adalah isi berkas \texttt{metadata-kel08.json} versi saat ini.
