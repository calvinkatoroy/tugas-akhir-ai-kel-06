...
\begin{tikzpicture}[
  ...
  \node[branch] (L) at ($(N.south)+(-2.55cm,-1.2cm)$) {
    \textbf{LSTM / GRU}\\[3pt]
    Sliding Window\\seq\_len = 10\\[2pt]
    output: $(n,\;10,\;16)$
  };
  \node[branch] (R) at ($(N.south)+(+2.55cm,-1.2cm)$) {
    \textbf{Random Forest}\\[3pt]
    Flat vector\\[2pt]
    output: $(n,\;16)$
  };
  \node[proc] (T) at ($(N.south)+(0,-3.3cm)$) {\textbf{Training:} CrossEntropyLoss / \texttt{RF.fit()} $\to$ simpan checkpoint terbaik};
...
\begin{tikzpicture}[
  ...
  startstop/.style={rectangle, rounded corners=12pt, draw=green!50!black,
                    ...
                    minimum height=0.75cm, font=\small\bfseries, inner sep=5pt},
  proc/.style={rectangle, draw=blue!60!black, fill=blue!7,
               text width=7cm, align=center,
               minimum height=0.75cm, font=\small, inner sep=5pt},
